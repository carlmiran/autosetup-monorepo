...
export interface DiagnosticoInput {
  ...
  /** Preço real dos principais serviços/produtos, informado pelo
   * próprio dono. Fonte: pedido de Carlos (31/07/2026) — quando isso
   * existe, o impacto financeiro deixa de ser estimativa vaga e vira
   * cálculo real (ex: "mais 4 clientes/mês x R$150 = R$600"). */
...
function linhasContextoProfundo(input: DiagnosticoInput): string {
  return [
    ...
    input.perdaFinanceira
      ...
      : null,
    input.precosServicos
      ? `- Preço real dos principais serviços/produtos (informado pelo dono): "${input.precosServicos}"`
      : null,
  ]
    .filter(Boolean)
    .join("\n");
...
SE "NICHO" NÃO PARECER UM NEGÓCIO DE VERDADE (ex: uma situação pessoal como "desempregado", "mãe solteira", ou qualquer coisa que não seja um tipo de negócio real): não finja que existe um negócio. No "resumo", diga isso com gentileza — que esse diagnóstico é pensado pra quem já tem ou está começando um negócio, e que o AutoSetup também tem um jeito de gerar renda sem ter negócio próprio (indicando negócios pro AutoSetup, sem precisar de CNPJ). Preencha os outros campos de forma mínima e honesta (não invente oportunidade de negócio que não existe).
